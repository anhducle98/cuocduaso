...
public:
    ...
    Rect get_bounding_box(Mat &object_binary);
    void lets_be_handsome(Mat &bgr_frame, Mat &gray_frame, Mat &topview_frame, Mat &binary_frame, Rect &obj_box);
    Line process_frame(Mat &bgr_frame, VideoWriter &bgr_writer, int &topview_height, int &topview_width, Rect &obj_box); // tra ve approximation-line theo frame
    double adjust_angle(Line previous_line, Line current_line, int topview_height, int topview_width, Rect &obj_box); //chinh lai goc mot ti
    ...
    double get_stupid_angle(Mat &bgr_frame, VideoWriter &bgr_writer);
};
